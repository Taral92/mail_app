        ...
        <div className="flex item-center gap-10  py-3">
             {
              inboxicons.map((items,index)=>(
                  <button onClick={()=>setselected(index)} className={`${selected===index ? 'border-b-4 border-b-blue-400 text-blue-500': 'border-b-transparent'} flex w-52 p-4 hover:bg-gray-200 items-center gap-2 cursor-pointer`}
                   key={index}>{items.icon} 
